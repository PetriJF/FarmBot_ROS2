...
class MapController(Node):
    ...
    def __init__(self):
        ...
        #                                            'map_info', self.map_info_server)

        self.cmd_callback_group = ReentrantCallbackGroup()

        # Initialise state servers
        self.get_map_trigger = self.create_service(Trigger, 'get_map',
                                                   self.get_map_server,
                                                   callback_group=self.cmd_callback_group)

        # Map command service server
        self.add_plant = self.create_service(AddPlant, 'add_plant', self.add_plant_server,
                                             callback_group=self.cmd_callback_group)
        self.update_map = self.create_service(UpdateMap, 'update_map', self.update_map_server,
                                              callback_group=self.cmd_callback_group)
        self.add_tool = self.create_service(AddTool, 'add_tool', self.add_tool_server,
                                            callback_group=self.cmd_callback_group)
        self.add_tray = self.create_service(AddSeedTray, 'add_seed_tray', self.add_tray_server,
                                            callback_group=self.cmd_callback_group)
        self.remove_map_object = self.create_service(RemoveMapObject, 'remove_map_object',
    ...
    # def add_plant(
    #         self, x: float, y: float, z: float, max_z: float, water_quantity: float,
    #         exclusion_radius: float, canopy_radius: float, plant_name: str, growth_stage: str
    #               ):
    #     """
    #     Handle adding plants to the active_map file.
    ...
    #     self.plant_ref['plant_details']['water_quantity'] = water_quantity
    ...
    #     """
    #     Create the command sequence for planting the seeds marked with the 'Planning' GrowthStage.
    ...
    #                 plant,
    #                 self.map_instance['map_reference']['trays'][tray_index]
    #             )
    ...
    #     """
    #     Create the sequence for planting a single seed.
    ...
    # def map_info_server(self, request, response):
    #     """
    #     Map Command Server.
    ...
    #         plant = plants[plant_index]
    #         water_pulses = (
    #             int(plant['plant_details']['water_quantity'])
    #             if rigid
    #             else map_moisture_reading(
    #                 reading=int(
    ...
    #     """Create the sequence for watering a single plant."""
    #     cmd = ''
    ...
    #     """Interpret commands around the seed trays."""
    #     elem = msg.split('_')
    #     index = int(elem[1])
    #     cmd = int(elem[2])
    ...
    #     """Interpret commands around the tools."""
    #     elem = msg.split('_')
    #     index = elem[1]
    #     cmd = int(elem[2][0])
    ...
    #     """
    #     Update soil moisture reading for a plant in the map.

    #     Args:
    #         index: The plant index to update.
    #         reading: The soil moisture reading value.
...
def main(args=None):
    """Initialize and run the MapController node."""
