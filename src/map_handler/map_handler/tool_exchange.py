...
class ToolExchanger:
    ...
    def __check_tool_details(self, cmd = ToolDetails):
        ...
        if not self.__outside_bounds(x_min = 0.0, x_max = self.map_max_x, y_min = 0.0, y_max = self.map_max_y, z_min = self.map_max_z, z_max = 0.0,
            ...
            self.node_.get_logger().warn(f"Tool release position {cmd.x_pos + cmd.release_x_inc} {cmd.y_pos + cmd.release_y_inc} {cmd.z_pos} is outside of the farmbot's reach!")
            return False
