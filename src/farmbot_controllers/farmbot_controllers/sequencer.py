...
class Sequencer:
    def __init__(self, node: Node, mvm: Movement, devices: DeviceControl):
        ...
        self.busy_state_sub = self.node.create_subscription(Bool, 'busy_state', self.status_callback, 10)
        self.sequencer_sub = self.node.create_subscription(String, 'sequencer', self.extend_sequence, 10)
    ...
    def peripheral4_pulses(self, delay = 500):
        ...
        '''
        peripheral4_pin = 10
        self.devices.set_pin_value_2(pin = peripheral4_pin, value1 = 1, delay = delay, value2 = 0, pin_mode = False)
    ...
    def sequencing_timer(self):
        ...
        if not self.farmbot_busy and not self.wait_for_request.wait_flag:
            ...
            if self.command_type == '':
                self.node.get_logger().warn(f"Command type not set! Not enough context! Command '{self.sequence[0]}' ignored")
                return
            ...
            if (self.wait_for_request.result != -1 and self.wait_for_request.wait_for == 63
                ...
                self.node.get_logger().info(f"Tool {'mounted' if self.wait_for_request.expected == 0 else 'unmounted'} successfully")
                ...
                self.wait_for_request.index = -1
                    ...
                    and self.wait_for_request.result != self.wait_for_request.expected):
                self.node.get_logger().warn(f"FAILED TOOL {'MOUNTING' if self.wait_for_request.expected == 0 else 'UNMOUNTING'}!! Stopping sequence")
                self.wait_for_request.expected = -1
                self.wait_for_request.result = -1
                self.wait_for_request.wait_for = -1
                self.wait_for_request.index = -1
            ...
            if(self.wait_for_request.result != -1 and self.wait_for_request.wait_for == 59):
                reading: str = f'SoilReading {self.wait_for_request.index} {self.wait_for_request.result}'
            ...
            # Manipulate the device as indicated in the command
                ...
                if cmd[0] == 'Vacuum':
                    if cmd[1] in ['0', '1']:
                        ...
                        self.node.get_logger().warn(f'Vacuum pump command has a state other than on or off. Command ignored!')
            ...
            # Handle Vision Commands
                ...
                # Vision command
                if cmd[0] == 'CALIB':
                    self.cam_calib_client(cmd = (self.sequence[0] + ' ' + str(self.x) + ' ' + str(self.y) + ' ' + str(self.z)))
                elif cmd[0] == 'PAN':
    ...
    def stitch_panorama_client(self, detect_weeds: bool, calib: bool, update_map: bool, mosaic: bool, x: float, y: float, z: float, num = int(-1)):
        ...
        if sum([calib, update_map, mosaic]) > 1:
            self.node.get_logger().warn('Cannot have more than 1 command type sent at the same time!')
