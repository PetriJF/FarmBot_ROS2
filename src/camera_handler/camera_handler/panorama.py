"""
Panorama stitching module for FarmBot.
...
class Panorama:
    """Take pictures and stitch camera images into panoramas representing map information."""
    ...
    def __init__(self, node: Node):
        ...
        self.depth_sub = self.node_.create_subscription(Image, '/depth_img',
                                                        self.__depth_callback, 10)
    ...
    def save_image_for_mosaic(self, num: int):
        """Save the current RGB image to the mosaic directory with a numbered filename."""
    ...
    def stitch_image_onto_map(self, x: float, y: float):
        """Stitch the camera image onto the panorama map at the specified coordinates.
        ...
        map_image = cv2.imread(map_path,
                               cv2.IMREAD_UNCHANGED)  # Ensures map_image is read with alpha channel
        ...
        start_y = (
            map_height - y_px - new_img_height // 2  # Note: map_height - y_px to flip y-coordinate
        )
        ...
        if roi_end_x > roi_start_x and roi_end_y > roi_start_y:
            ...
            for c in range(0, 3):
                map_image[
                    roi_start_y:roi_end_y, roi_start_x:roi_end_x, c
                    ] = (alpha_s * processed_image[new_start_y:new_end_y, new_start_x:new_end_x, c]
                         + alpha_l * map_image[roi_start_y:roi_end_y, roi_start_x:roi_end_x, c])

            map_image[
                roi_start_y:roi_end_y, roi_start_x:roi_end_x, 3
                ] = (alpha_s * processed_image[new_start_y:new_end_y, new_start_x:new_end_x, 3] +
    ...
    def rotate_and_mask_image(self, image, angle):
        """Rotate the image around the center at the set angle and adds a transparent mask."""
        ...
        alpha_channel = np.ones((rotated_image.shape[0], rotated_image.shape[1], 1),
                                dtype=rotated_image.dtype) * 255
        ...
        cv2.fillConvexPoly(
            mask,
            cv2.boxPoints(
                ((width // 2, height // 2), (width, height), angle)
                ).astype(int), (255, 255, 255, 255)
            )
    ...
    def get_panorama_increments(self):
        """Return panorama increments for x and y axes.

        Compute optimal image overlap to minimize the number of images required
        to stitch the full map into a panorama.
        """
