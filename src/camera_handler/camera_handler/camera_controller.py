#!/usr/bin/env python3
"""
ROS2 CameraController node for FarmBot.
...
class CameraController(Node):
    """ROS2 node for FarmBot camera operations: panorama, calibration, and plant detection."""

    # Node contructor
    def __init__(self):
        """Initialize the CameraController node and camera helper modules."""
        ...
        self.panorama_sequencing_server_ = self.create_service(StringRepReq, 'panorama_sequence',
                                                               self.panorama_server_callback)

        # Sequencing Service Server
        self.panorama_server_ = self.create_service(StringRepReq, 'form_panorama',
                                                    self.stitch_image_server)
        # self.take_picture_ = LuxonisCameraNode(self)

        # Camera Calibration Server
        self.calibration_server_ = self.create_service(StringRepReq, 'camera_calibration',
                                                       self.calibration_server_callback)
        ...
        if request.data == 'CALIB':
            ...
            if float(msg[2]) != 0.0:    # Ensuring the z-axis is homed
                self.get_logger().warn(
                    'Z Axis is not in home position for the panorama stitching!'
                    'Panorama command cancelled'
                    )
                response.data = 'FAILED'
                return response
    ...
    def calibration_server_callback(self, request, response):
        ...
        if request.data == 'GET':   # Get calibration sequence
            ...
            response.data = self.calib_.calibrate_camera(run=int(info[1]), x=float(info[2]),
                                                         y=float(info[3]), z=float(info[4]))
    ...
    def panorama_server_callback(self, request, response):
        ...
        Creates a sequence for making taking pictures at multiple positions through the farmbot
        working area and stitching them into a large panorama.
        """
        seq = ''
        ...
        if self.panorama_.map_x != -1.0 and self.panorama_.map_y != -1.0:
            self.get_logger().info(
                f'Panorama max_x: {self.panorama_.map_x}, map_y: {self.panorama_.map_y}')
            ...
            for y_pos in range(y_pos_count + 1):
                ...
                for x_pos in x_range:
                    ...
                    y = int(y_pos * y_inc)
                    seq += f'CC_P\n{x} {y} 0.0\n'
                    # Conditionally add either a mosaic or panoramic command based on request.data
...
def main(args=None):
    """Initialize ROS2 and run the CameraController node until shutdown."""
