...
class CalibrateCamera:
    ...
    def __init__(self, node: Node):
        """
        Calibration module constructer.

        Initializes variables and sets subscriptions
        """
        self.node_ = node
    ...
    def get_sequence(self):
        """Form the command sequence used for calibrating the camera."""
        cmd_seq = ''
        run_count = 1
        seconds = 5
    ...
    def capture(self, run: int, x: float, y: float, z: float):
        """Once at the desired position, an image is captured from the luxonis camera feed."""
    ...
    def find_pattern(self, img, save_output=None):
        ...
        if not ret and save_output is None:
            self.node_.get_logger().warn(
                'ERROR: Calibration failed, calibration object not detected in image.'
                'Check recent photos.'
                )
            self.success_flag = False
        return ret, centers
    ...
    def get_panorama_increments(self):
        """
        Get the panorama increments for the x and y axis.

        Used for minimizing the amount of images needed to stitch the whole map into a panorama.
        """
