"""
Module for Luxonis DepthAI camera streaming.
...
class LuxonisCameraNode(Node):
    ...
    def stop_processing(self):  # Gracefully stop the processing thread
        """Stop the packet-processing thread cleanly and wait for it to finish."""
    ...
    def __del__(self):  # Consider removing if you opt for explicit shutdown calls
        """Ensure packet-processing stops and resources are cleaned up."""
        self.stop_processing()  # Ensure resources are cleaned up

    def configure_depthai_components(self):
        """Configure the DepthAI components."""
    ...
    def process_depth_frame(self, disparity_frame):
        ...
        scaled_frame = np.clip(
            ((depth_frame - 0) / (self.DEPTH_MAX_VAL - 0) * 255.0), 0, 255
            ).astype(np.uint8)
        return scaled_frame
